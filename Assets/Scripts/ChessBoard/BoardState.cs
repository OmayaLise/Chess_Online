...
public partial class ChessGameManager
{
    ...
    public class BoardState
    {
        ...
        public string BoardToString()
        {
            ...
            boardState += instance.GetTeamTurn() == EChessTeam.White ? 'W' : instance.GetTeamTurn() == EChessTeam.Black ? 'B' : ' ';
            boardState += '@' + instance.GetScore(EChessTeam.White).ToString() + '@' + instance.GetScore(EChessTeam.Black).ToString() + '@';

            for (int i = 0; i < squares.Count; ++i)
            {
                boardState += TeamChar(squares[i]);
                boardState += PieceChar(squares[i]);
        ...
        char TeamChar(BoardSquare square)
        {
            switch (square.team)
            {
        ...
        char PieceChar(BoardSquare square)
        {
            switch (square.piece)
            {
        ...
        public void SetupBoard(string boardState, EChessTeam curTeam, uint whiteScore, uint blackScore)
        {
            instance.SetScore(whiteScore, blackScore);
            instance.SetTeamTurn(curTeam);

            string[] squareInfo = boardState.Split('|');
            for (int i = 0; i < squares.Count; ++i)
            {
                EChessTeam team = EChessTeam.None;
                EPieceType pieceType = EPieceType.None;
                switch (squareInfo[i][0])
                {
                    case 'W':
                ...
                SetPieceAtSquare(i, team, pieceType);
            }
            if (instance.OnPlayerTurn != null)
                instance.OnPlayerTurn(instance.teamTurn == EChessTeam.White);
        }
    }
}
